...
export default async function PlaysPage() {
    ...
    // 1. 모든 공개 프로젝트 설정 조회 (category 컬럼이 없을 경우 대비하여 인메모리 필터링)
    ...
    const initialProjects = (configs ?? [])
        ...
        .filter((p) => p.meta && p.config.category === 'plays') as { config: ProjectConfig; meta: ProjectMeta }[];
            ...
            <main className="flex-1">
                <Container className="py-12 lg:py-16 space-y-8">
                    <div className="space-y-1">
                        <h4 className="text-2xl font-semibold tracking-tight">재밌게 즐겨주세요~^^</h4>
                    </div>

                    <ProjectListClient initialProjects={initialProjects} baseUrl="/plays" />
                </Container>
